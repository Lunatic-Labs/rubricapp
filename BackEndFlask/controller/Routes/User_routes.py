...
def add_user():
    if(request.args and request.args.get("team_id")):
        ...
        if type(team)==type(""):
            print(f"[User_routes /user?team_id=<int:team_id> POST] An error occurred retrieving team_id: {team_id}, ", team)
            createBadResponse(f"An error occurred retrieving team_id: {team_id}!", team, "users")
            return response
        ...
        for user_id in user_ids:
            team_user = add_user_to_team(int(user_id), team_id)

            if type(team_user)==type(""):
                print(f"[User_routes /user?team_id=<int:team_id> POST] An error occurred adding user_id:{user_id} to team_id: {team_id}, ", team_user)
                createBadResponse(f"An error occurred adding user_id:{user_id} to team_id: {team_id}!", team_user, "users")
                return response

        print(f"[User_routes /user?team_id=<int:team_id> POST] Successfully added users to the team_id: ${team_id}!")
        createGoodResponse(f"[User_routes /user?team_id=<int:team_id> POST] Successfully added users to the team_id: {team_id}!", [], 201, "users")
        return response
...
def updateUser():
    if (request.args and request.args.get("team_id")):
        ...
        for user_id in user_ids:
            team_user = remove_user_from_team(int(user_id), team_id)
