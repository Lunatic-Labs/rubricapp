...
def get_student_individual_ratings(): 
    ...
    if assessment_task_id == type(""):
        print(
            f"[Rating_routes.py /rating GET] An error occurred retrieving all ratings for assessment_task_id: {assessment_task_id}"
        )
        createBadResponse(
            f"An error occurred retrieving all ratings for assessment_task_id: {assessment_task_id}!",
            assessment_task_id, "Ratings"
        )
        return response
    ...
    createGoodResponse(
        "Successfully retrieved all individual ratings!",
        name_rating_schema.dump(data),
        200, "ratings"
    )
    return response
