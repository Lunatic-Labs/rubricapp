...
def test_one_ta_ten_students(flask_app_mock: type) -> None:
...
def test_no_ta_ten_students(flask_app_mock: type) -> None:
    with flask_app_mock.app_context():
        try:
...
##       - assigns all of the 10 students to a team
def test_ten_tas_ten_students(flask_app_mock: type) -> None:
    with flask_app_mock.app_context():
        try:
            result = create_one_admin_ta_student_course()
...
def test_TA_true_but_no_TAs_recorded_error(flask_app_mock: type) -> None:
    with flask_app_mock.app_context():
        try:
