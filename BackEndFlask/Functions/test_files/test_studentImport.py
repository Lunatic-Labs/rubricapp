...
def test_WrongFormat(flask_app_mock):
    with flask_app_mock.app_context():
        create_testcourse(False)
        assert studentcsvToDB(retrieveFilePath("WrongFormatRoster.csv"), 1, 1) == "Row other than header does not contain an integer where an lms_id is expected. Misformatting Suspected."
...
def test_TooManyColumns(flask_app_mock):
    with flask_app_mock.app_context():
        create_testcourse(False)
        assert studentcsvToDB(retrieveFilePath("TooManyColRoster.csv"), 1, 1) == "File contains more than the 3 expected columns: \"last_name, first_name\", lms_id, email"
...
def test_NotEnoughCol(flask_app_mock):
    with flask_app_mock.app_context():
        create_testcourse(False)
        assert studentcsvToDB(retrieveFilePath("NotEnoughColRoster.csv"), 1, 1) == "File contains less than the 3 expected columns: \"last_name, first_name\", lms_id, email"
