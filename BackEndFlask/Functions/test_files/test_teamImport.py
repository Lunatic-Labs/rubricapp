...
def test_valid_file_wTAs_records_all_data(flask_app_mock):
    with flask_app_mock.app_context():
        ...
        teamcsvToDB(retrieveFilePath("ValidTeamsYesTAs.csv"), 2, 1)
...
def test_valid_file_woTAs_records_all_data(flask_app_mock):
    with flask_app_mock.app_context():
        ...
        teamcsvToDB(retrieveFilePath("ValidTeamsNoTAs.csv"), 2, 1)
        assert get_teams().__len__()==5 and get_team_users().__len__()==20 and get_team_courses().__len__()==5
...
def test_file_not_found_error(flask_app_mock):
    with flask_app_mock.app_context():
        ...
        assert teamcsvToDB(retrieveFilePath("NonExistentFile.csv"), 2, 1) == "File not found or does not exist!"
...
def test_wrong_file_type_error(flask_app_mock):
    with flask_app_mock.app_context():
        create_test_user_course(20)
        assert teamcsvToDB(retrieveFilePath("WrongFileType.pdf"), 2, 1) == "Wrong filetype submitted! Please submit a .csv file."
...
def test_misformatting_TA_email_error(flask_app_mock):
    with flask_app_mock.app_context():
        create_test_user_course(20, True, 3)
        assert teamcsvToDB(retrieveFilePath("WrongFormattingTeamsYesTAs.csv"), 2, 1) == "Row does not contain an email where an email is expected. Misformatting Suspected."
...
def test_misformatting_student_email_error(flask_app_mock):
    with flask_app_mock.app_context():
        create_test_user_course(20)
        assert teamcsvToDB(retrieveFilePath("WrongFormattingTeamsNoTAs.csv"), 2, 1) == "Row does not contain an email where an email is expected. Misformatting Suspected."
...
def test_users_do_not_exist_error(flask_app_mock):
    with flask_app_mock.app_context():
        ...
        assert teamcsvToDB(retrieveFilePath("ValidTeamsNoTAs.csv"), 2, 1) == expectedError
...
def test_TA_not_yet_added_error(flask_app_mock):
    with flask_app_mock.app_context():
        ...
        assert teamcsvToDB(retrieveFilePath("ValidTeamsYesTAs.csv"), 2, 1) == expectedError
