...
def test_valid_file_wTAs_records_all_data(flask_app_mock):
    with flask_app_mock.app_context():
        ...
        usesTAs = True
        numofTAs = 3
        teamfile = os.getcwd() + os.path.join(os.path.sep, "Functions") + os.path.join(os.path.sep, "sample_files")
        teamfile += os.path.join(os.path.sep, "ValidTeamsYesTAs.csv")
        create_test_user_course(numOfStudents, usesTAs, numofTAs)
        teamfileToDB(teamfile, owner_id, course_id)
...
def test_valid_xlsx_file_wTAs_records_all_data(flask_app_mock):
    with flask_app_mock.app_context():
        ...
        numofTAs = 3
        teamfile = os.getcwd() + os.path.join(os.path.sep, "Functions") + os.path.join(os.path.sep, "sample_files")
        teamfile += os.path.join(os.path.sep, "ValidTeamsYesTAs.xlsx")
        create_test_user_course(numOfStudents, usesTAs, numofTAs)
        teamfileToDB(teamfile, owner_id, course_id)
        teams = Team.query.count()
        teamUsers = TeamUser.query.count()
...
def test_valid_file_woTAs_records_all_data(flask_app_mock):
    with flask_app_mock.app_context():
        ...
        numOfStudents = 20
        teamfile = os.getcwd() + os.path.join(os.path.sep, "Functions") + os.path.join(os.path.sep, "sample_files")
        teamfile += os.path.join(os.path.sep, "ValidTeamsNoTAs.csv")
        create_test_user_course(numOfStudents)
        teamfileToDB(teamfile, owner_id, course_id)
        teams = Team.query.count()
        teamUsers = TeamUser.query.count()
        teamCourses = TeamCourse.query.count()
...
def test_file_not_found_error(flask_app_mock):
    with flask_app_mock.app_context():
        ...
        numOfStudents = 20
        teamfile = os.getcwd() + os.path.join(os.path.sep, "Functions") + os.path.join(os.path.sep, "sample_files")
        teamfile += os.path.join(os.path.sep, "NonExistentFile.csv")
        create_test_user_course(numOfStudents)
        assert teamfileToDB(teamfile, owner_id, course_id) == "File not found or does not exist!"
...
def test_wrong_file_type_error(flask_app_mock):
    with flask_app_mock.app_context():
        ...
        numOfStudents = 20
        teamfile = os.getcwd() + os.path.join(os.path.sep, "Functions") + os.path.join(os.path.sep, "sample_files")
        teamfile += os.path.join(os.path.sep, "WrongFileType.pdf")
        create_test_user_course(numOfStudents)
        assert teamfileToDB(teamfile, owner_id, course_id) == "Wrong filetype submitted! Please submit a .csv file."
...
def test_misformatting_TA_email_error(flask_app_mock):
    with flask_app_mock.app_context():
        ...
        numOfTAs = 3
        teamfile = os.getcwd() + os.path.join(os.path.sep, "Functions") + os.path.join(os.path.sep, "sample_files")
        teamfile += os.path.join(os.path.sep, "WrongFormattingTeamsYesTAs.csv")
        create_test_user_course(numOfStudents, usesTAs, numOfTAs)
        assert "The following row does not contain a valid email where email is expected:" in teamfileToDB(teamfile, owner_id, course_id)
...
def test_misformatting_student_email_error(flask_app_mock):
    with flask_app_mock.app_context():
        ...
        numOfStudents = 20
        teamfile = os.getcwd() + os.path.join(os.path.sep, "Functions") + os.path.join(os.path.sep, "sample_files")
        teamfile += os.path.join(os.path.sep, "WrongFormattingTeamsNoTAs.csv")
        create_test_user_course(numOfStudents)
        assert "The following row does not contain a valid email where email is expected:" in teamfileToDB(teamfile, owner_id, course_id)
...
def test_users_do_not_exist_error(flask_app_mock):
    with flask_app_mock.app_context():
        ...
        assert teamfileToDB(teamfile, owner_id, course_id) == expectedError
