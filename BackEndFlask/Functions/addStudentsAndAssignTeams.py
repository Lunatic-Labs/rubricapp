...
def student_and_team_to_db(roster_file: str, owner_id: int, course_id: int):
    ...
    for team in teams:
        ...
        if not helper_ok(deactivated_team):
            return helper_cleanup(roster_file, is_xlsx, deactivated_team, student_and_team_csv, save_point=save_point)
        ...
        if course_uses_tas:
            ...
            if not helper_ok(ta_user):
                save_point.rollback()
                return helper_cleanup(roster_file, is_xlsx, ta_user, student_and_team_csv, save_point=save_point)
            ...
            if not helper_ok(ta_user_id):
                save_point.rollback()
                return helper_cleanup(roster_file, is_xlsx, ta_user_id, student_and_team_csv, save_point=save_point)
    ...
    for student_info in roster[1:]:
        ...
        if not helper_ok(new_team):
            save_point.rollback()
            return helper_cleanup(roster_file, is_xlsx, new_team, student_and_team_csv, save_point=save_point)
        ...
        if course_uses_tas:
            ...
            if not helper_ok(ta_team_user):
                save_point.rollback()
                return helper_cleanup(roster_file, is_xlsx, ta_team_user, student_and_team_csv, save_point=save_point)
