...
def genericcsv_to_db(user_file: str, owner_id: int, course_id: int):
    ...
    # Begin save point so we can revert changes in the db if an error occurrs.
    ...
    for row in range(0, len(roster)):
        ...
        if len(person_attribs) < min_person_attribs_count:
            save_point.rollback()
            return helper_cleanup(user_file, is_xlsx, NotEnoughColumns.error, student_csv, save_point=save_point)

        if len(person_attribs) > max_person_attribs_count:
            save_point.rollback()
            return helper_cleanup(user_file, is_xlsx, TooManyColumns.error, student_csv, save_point=save_point)
        ...
        if not helper_ok(role, user_file, is_xlsx):
            save_point.rollback()
            return helper_cleanup(user_file, is_xlsx, role, student_csv, save_point=save_point)
        ...
        if not helper_verify_email_syntax(email):
            save_point.rollback()
            return helper_cleanup(user_file, is_xlsx, SuspectedMisformatting.error, student_csv, save_point=save_point)
        ...
        if not helper_ok(user, user_file, is_xlsx):
            save_point.rollback()
            return helper_cleanup(user_file, is_xlsx, user, student_csv, save_point=save_point)
        ...
        if user is None:
            ...
            if not helper_ok(created_user, user_file, is_xlsx):
                save_point.rollback()
                return helper_cleanup(user_file, is_xlsx, created_user, student_csv, save_point=save_point)

        user_id = get_user_user_id_by_email(email)
        if not helper_ok(user_id, user_file, is_xlsx):
            save_point.rollback()
            return helper_cleanup(user_file, is_xlsx, user_id, student_csv, save_point=save_point)

        user_course = get_user_course_by_user_id_and_course_id(user_id, course_id)
        if not helper_ok(user_course, user_file, is_xlsx):
            save_point.rollback()
            return helper_cleanup(user_file, is_xlsx, user_course, student_csv, save_point=save_point)
    ...
    return helper_cleanup(user_file, is_xlsx, None, student_csv, save_point=save_point)
