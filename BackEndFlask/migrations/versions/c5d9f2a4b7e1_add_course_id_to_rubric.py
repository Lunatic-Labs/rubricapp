...
def upgrade():
    # Add the course the rubric is designated for. Nullable because legacy
    # rubrics predate course scoping.
    op.add_column(
        'Rubric',
    ...
    # Backfill from the first assessment task that uses each rubric, so
    # existing rubrics land in the course they were used in.
    op.execute(
        """
        UPDATE Rubric
        SET course_id = (
