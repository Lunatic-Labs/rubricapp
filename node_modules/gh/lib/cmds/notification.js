...
async function latest(options, opt_watch) {
    ...
    try {
        var data = await options.GitHub.paginate(options.GitHub.activity.listRepoEvents.endpoint(payload));
    }
    catch (err) {
    ...
    if (filteredListEvents.length) {
        ...
        filteredListEvents.forEach(event => {
            logger.log(`${logger.colors.magenta(`@${event.actor.login}`)} ${event.txt} ${logger.colors.cyan(options.repo)} ${logger.getDuration(event.created_at, options.date)}`);
        });
    }
}
