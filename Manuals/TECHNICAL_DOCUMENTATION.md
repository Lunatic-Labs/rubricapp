...
- Additional tooling: Dockerfiles, a compose file for local multi-container runs, and database scripts.
...
Database
- The repository supports MySQL DBs in development and production.
- `BACKUP_SQL_INSTRUCTIONS.md` and `dbinsert.sh` provide guidance for preserving and populating DB data.

Authentication
- The back end uses Flask extensions and JWT to handle the authentication flows; the front end will use the auth tokens to access protected API endpoints.
...
4. The database stores users, rubrics, evaluations, and other persistent data. For development and production deployments the system uses a MySQL database.
...
Final notes
-----------
This document provides a practical, entry-level introduction to RubricApp’s codebase, its languages, and the major libraries and tools you will encounter. To get exact versions and the full list of packages used in the project, open the dependency files located inside FrontEndReact (package.json) and BackEndFlask (requirements.txt or equivalent). For database operations follow the included  BACKUP_SQL_INSTRUCTIONS.md files in the repository.
