...
class AdminAddUser extends Component {
  constructor(props) {
    ...
    this.unenrollUser = () => {
      ...
      genericResourcePUT(
        `/user?uid=${navbar.state.user["user_id"]}&course_id=${
...
>>>>>>> 6f088fc611b997eaa51900909e94229d1a7007fd
      return;
    }
    ...
    if (id === "lmsId") {
      ...
      if (
        typeof MAX_LMS_ID_LENGTH === "number" &&
        value.length > MAX_LMS_ID_LENGTH
      ) {
        this.setState({
          errors: {
            ...this.state.errors,
            [id]: `Max ${MAX_LMS_ID_LENGTH} digits.`,
          },
        });
        return; // don’t update value
      }
      ...
      this.setState({
        ...
        errors: {
          ...this.state.errors,
          [id]: atMax ? `Max ${MAX_LMS_ID_LENGTH} digits reached.` : "",
        },
      });
      return;
    }
  ...
  handleSubmit = () => {
    const { firstName, lastName, email, originalEmail, role, lmsId } =
      this.state;
...
>>>>>>> 6f088fc611b997eaa51900909e94229d1a7007fd
    ...
    // Case 3: Non-super admin adding a new user
      ...
      promise = genericResourcePOST(
        `/user?course_id=${chosenCourse["course_id"]}&owner_id=${owner_id}`,
        this,
        body
      );
    }
    // Case 4: Non-super admin editing an existing user
      ...
      promise =
        email !== originalEmail
          ? genericResourcePUT(
              `/user?uid=${user["user_id"]}&course_id=${chosenCourse["course_id"]}&new_email=${email}&owner_id=${owner_id}`,
              this,
              body
            )
          : genericResourcePUT(
              `/user?uid=${user["user_id"]}&course_id=${chosenCourse["course_id"]}`,
              this,
              body
            );
    }
    ...
    promise
      .then((result) => {
        ...
        if (result && typeof result.errorMessage === "string") {
          ...
          const isMysqlDup =
            /\b1062\b.*duplicate entry/i.test(msg) && /email/i.test(msg);
          const isPgDup =
            /duplicate key value/i.test(msg) &&
            /unique constraint/i.test(msg) &&
            /email/i.test(msg);
          const isSqliteDup =
            /UNIQUE constraint failed/i.test(msg) && /email/i.test(msg);
  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        {validMessage !== "" && (
          <ErrorMessage add={state.addUser} error={validMessage} />
        )}
            ...
            <Box className="card-style">
              <FormControl className="form-spacing" aria-label="addUserForm">
                ...
                <Box className="form-input">
                  ...
                  <Box id="junkReminder" sx={{ mb: 3 }}>
                    <b className="primary-color-text">
                      Make sure students check their junk/spam folder for the
                      invitation!
                    </b>
                  </Box>
                  ...
                  >
                    ...
                    <Button
                      ...
                      variant="contained"
                      aria-label="addOrSaveAddUserButton"
                    >
                      {editUser ? "Update User" : "Add User"}
                    </Button>
                  </Box>
