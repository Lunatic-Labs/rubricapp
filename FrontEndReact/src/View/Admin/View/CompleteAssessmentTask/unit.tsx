...
export class FixedTeamUnit extends ATUnit {
	...
	getCheckedInTooltip(checkinsTracker: any) {
		...
		if (checkedInMembers.length !== 0) {
			return checkedInMembers.map((user: { first_name: string; last_name: string }, index: number) => <Box key={index}>{user["first_name"] + " " + user["last_name"]}</Box>);
		} else {
