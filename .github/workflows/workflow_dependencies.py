...
with open('BackEndFlask/requirements.txt', 'r') as req_file:
    req = sorted([line.replace(" ", "").strip().lower() for line in req_file if line.strip() and not line.startswith("#")])
with open('temp_dependencies.txt', 'r') as current_file:
    current = sorted([line.strip().lower() for line in current_file if line.strip()])
